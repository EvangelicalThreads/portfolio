    ...
    <>
      <nav className="fixed top-0 left-0 right-0 bg-white/75 backdrop-blur-md z-50 border-b border-neutral-200">
        <div className="mx-auto max-w-7xl px-5 md:px-8 py-4 flex items-center justify-between">
          <button onClick={() => smoothScrollTo("home")} className="text-[17px] md:text-lg font-semibold tracking-tight text-neutral-900">
            Katie Wang
          </button>
      ...
      >
        <button className="self-end text-3xl mb-8" onClick={() => setMobileOpen(false)}>✕</button>
        <ul className="flex flex-col space-y-6 text-lg font-semibold">
          {links.map((link) => (
            <li key={link.id}>
              <button
                onClick={() => { smoothScrollTo(link.id); setMobileOpen(false); }}
                className={classNames(
                  "transition-colors hover:text-black focus:outline-none focus-visible:ring-2 focus-visible:ring-black/30 rounded-sm px-1"
                )}
              >
...
export default function ProjectSlugPage() {
  ...
  const formattedSlug = typeof slug === "string" ? slug.replace(/-/g, " ") : "";

  return (
    <div className="min-h-screen bg-neutral-50">
      <ProjectsNavbar projects={projects} />
      <main className="pt-24 max-w-7xl mx-auto px-5 md:px-8">
        <h1 className="text-4xl font-bold mb-6">{formattedSlug}</h1>
        <p className="text-lg text-neutral-700">
          This is the detailed page for <strong>{slug}</strong>. Add your project description, images, and any relevant content here.
        </p>
      </main>
    </div>
  );
